...
public class HW2Operations
{
    ...
    {
        return (~(0x000000FF << (which << 3))) & num | (a_byte << (which << 3));
    }
    ...
    {
        return ((0x0000000F << (which << 2)) & num) >> (which << 2);
    }
    ...
    {
        if ((num & (1 << (n - 1))) != 0) {
            //zero out past the sign bit because num is technically 32 bit
            return ((~(0xFFFFFFFF << n) & ~num ) + 1);
        }
        return num;
    }
    ...
    {
        return (((num & (num - 1)) == 0) && ((((num | (num - 1)) >> 31) & 0x00000001) != 1));
    }
}
